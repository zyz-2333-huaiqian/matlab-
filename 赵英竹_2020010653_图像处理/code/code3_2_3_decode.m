...
C(2:end,:) = AC;
secret_decode = [];
for m = 1:width*height/64
    n = 64;
    while (C(n,m) == 0 && n>0)
        n = n-1;
    end
    if(C(n,m) == -1)
        secret_decode = [secret_decode,'0'];
    else
        secret_decode = [secret_decode,'1'];
    end
end
...
MSE_pre = (double(hall_gray) - pic).^2;
MSE = sum(sum(MSE_pre))/(120*168);
PSNR = 10*log10(65025/MSE)

load('secret.mat')
correct = 1-sum(abs(secret(1:length(secret_decode))-secret_decode))/length(secret_decode)
